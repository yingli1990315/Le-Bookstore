...
public class CheckoutController {
	...
	public String checkout(
		...
		List<CartItem> cartItemList = cartItemService.findByShoppingCart(user
				.getShoppingCart());
		...
		for (UserPayment userPayment : userPaymentList) {
			if (userPayment.isDefaultPayment()) {
				...
				billingAddressService.setByUserBilling(
						userPayment.getUserBilling(), billingAddress);
			}
		}
	...
	public String setShippingAddress(
			...
			Principal principal, Model model) {
		User user = userService.findByUsername(principal.getName());
		UserShipping userShipping = userShippingService
				.findById(userShippingId);

		if (userShipping.getUser().getId() != user.getId()) {
			return "badRequsetPage";
		} else {
			...
			List<CartItem> cartItemList = cartItemService
					.findByShoppingCart(user.getShoppingCart());
